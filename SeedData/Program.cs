...
// var prestamo = await db.Prestamos.Where(p => p.IDPrestamo == 7 && p.FechaAprobacion != null).FirstOrDefaultAsync();
// if (prestamo is null) {
...
// When
var ingresos = await db.Prestamos
                    .Where(p => (p.FechaAprobacion != null)
                                && (p.FechaLiquidacion == null))
                    .SumAsync(p => p.PagoMensual);
// Then
WriteLine($"Ingresos : {ingresos}");